...
const (
	...
	// backoffReset — сколько должно прожить соединение, чтобы следующая
	// пауза перед переподключением снова стала короткой.
...
// Reconnect закрывает текущее соединение, чтобы поздороваться заново.
//
// Единственный способ получить свежий logic.xml: описание дома сервер
// присылает только в ответ на рукопожатие, отдельного запроса нет ни в
// библиотеке, ни в самих кодах команд протокола. Цикл [Client.Run]
// поднимет соединение сам, и новое описание приедет вместе с ним.
//
// MQTT при этом не трогается: перезапускать шину ради списка элементов
// умного дома незачем.
func (c *Client) Reconnect() error {
	...
	if client == nil {
		return fmt.Errorf("соединения с умным домом нет: перечитывать нечего")
	}
	if err := client.Close(); err != nil {
		return fmt.Errorf("закрытие соединения: %w", err)
	}
	c.log.Info("переподключаемся к умному дому за свежим logic.xml")
	return nil
}
...
func (c *Client) Run(ctx context.Context) error {
	...
	for {
		...
		// Соединение, прожившее заметное время, — это успех, а не череда
		// неудач: следующий обрыв должен начинаться с короткой паузы. Без
		// сброса пауза росла до потолка за сутки работы, и переподключение
		// ради свежего logic.xml ждало бы минуту на ровном месте.
