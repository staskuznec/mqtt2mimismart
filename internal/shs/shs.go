...
const (
	// keepalive — период поддержания соединения. Значение эталонного клиента:
	// сервер выкидывает молчащих.
	keepalive = shclient.ReferenceKeepalive

	// idleTimeout — сколько ждём пакетов, прежде чем считать связь оборванной.
	// Втрое больше периода поддержания: одиночная потеря не должна ронять
	// соединение, а вот три подряд — уже повод переподключиться.
	idleTimeout = 3 * keepalive

	// syncWindow — окно, в течение которого события после подключения
	// считаются первичной синхронизацией, а не живыми изменениями.
	syncWindow = 5 * time.Second
...
func (c *Client) session(ctx context.Context) error {
	...
	if err := client.RequestAll(ctx); err != nil {
		return fmt.Errorf("запрос состояний: %w", err)
	}

	// Окно синхронизации закрывается по времени, а не по последнему пакету:
	// сколько именно их придёт, заранее неизвестно, а ждать тишины дольше
	// нужного — значит пропустить настоящие нажатия мимо шлюза.
	go func() {
		select {
		case <-ctx.Done():
		case <-time.After(syncWindow):
			...
			c.mu.Unlock()
			c.log.Debug("первичная синхронизация завершена")
		}
	}()
...
func (c *Client) dial(ctx context.Context) (*shclient.Client, *bytes.Buffer, error) {
	...
	client, err := shclient.New(cfg.Addr, cfg.Key,
		...
		shclient.WithKeepalive(keepalive),
