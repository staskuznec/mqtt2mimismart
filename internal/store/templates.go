...
//
// Порядок такой, чтобы отказ не оставлял мусора: сперва проверка назначений,
// потом устройство, и только потом связки. Половина развёрнутого устройства —
// это связки, которые уже работают, при том что устройства как будто нет.
...
func (s *Store) ReapplyTemplate(ctx context.Context, d Device, t devtmpl.Template,
	...
	//
	// Читаем прежний профиль до записи устройства: после неё в базе уже стоит
	// новый ключ, и узнать, чем устройство жило раньше, будет неоткуда.
	//
	// Файла прежнего профиля может и не быть — его удалили на странице
	// «Профили». Тогда убирать нечего: имена связок известны только из файла.
	// Молчать об этом нельзя, но и отказывать в смене профиля из-за пропавшего
	// файла тоже, поэтому предупреждает веб — там есть журнал.
	...
	// Устройство обновляем ДО удаления связок, и это не вкусовщина: у префикса
	// топиков уникальный индекс, и правка с чужим префиксом валит UPDATE. Стой
	// он после удаления — форма показала бы ошибку, а связки устройства были бы
	// уже стёрты. Та же беда при любом другом отказе: устройство удалили в
	// соседней вкладке, база не отвечает.
	d.Model, d.Template = t.Model, t.Key
