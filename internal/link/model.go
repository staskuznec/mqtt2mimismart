...
type Link struct {
	...
	// ToggleOnPress переключает элемент по нажатию в приложении (только In,
	// только однобайтовая форма).
	//
	// Нужно тревогам, которые устройство зажигает и не гасит. Датчик протечки
	// на батарейках спит: воду вытерли, а «протечка» горит до его следующего
	// пробуждения, и снять её нечем — виртуальный элемент по нажатию сам не
	// меняется, статус ему обязан прислать тот, кто его ведёт. Раньше это
	// делал скрипт умного дома, теперь может шлюз: нажали — записал обратное
	// тому, чем элемент сейчас живёт.
	ToggleOnPress bool
}
...
func (l Link) validateIn() error {
	...
	// Переключать нажатием можно только то, у чего есть два состояния: элемент
	// с однобайтовым статусом. У датчика 8.8 и строки «обратного значения» не
	// существует, и кнопка в приложении делала бы не то, что обещает.
	if l.ToggleOnPress && l.Encode != EncodeByte {
		return fmt.Errorf("переключение нажатием возможно только для однобайтовой формы, а не %q", l.Encode)
	}
	return nil
}
...
func (l Link) validateOut() error {
	if l.ToggleOnPress {
		return fmt.Errorf("переключение нажатием задаётся у связки «в дом»: связка «из дома» и так получает нажатие")
	}
