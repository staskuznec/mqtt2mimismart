...
func (h House) FreeSubIDs(id uint16, n int, from uint8) ([]uint8, error) {
	...
	// Не хватило — считаем, сколько свободно на модуле целиком и с какого
	// адреса начинается ближайшая дыра. Без этого сообщение «свободно 5,
	// нужно 10» непонятно: адресов на модуле может оставаться полсотни, все
	// они ниже указанного начала.
	free, first := 0, -1
	...
	if free < n {
		return nil, fmt.Errorf("на модуле %d свободно всего %d адресов, нужно %d: "+
			"заводите элементы на другом модуле", id, free, n)
	}
	return nil, fmt.Errorf("начиная с %d на модуле %d свободно %d адресов, нужно %d; "+
		"всего на модуле свободно %d — начните с %d",
		from, id, len(out), n, free, first)
}

// NextFreeSubID подсказывает, с какого адреса начинать: первый свободный.
//
// Раньше отсчёт шёл от последнего занятого — «чтобы не лезть в чужие дырки».
// На живом объекте это ровно наоборот: у модуля с элементами на 250-м адресе
// оставалось пять мест, а полсотни свободных между областями не использовались
// вовсе, и заведение падало с отказом. Разрывы в нумерации — обычное дело, и
// заполнять их безопасно: адрес с областью никак не связан.
