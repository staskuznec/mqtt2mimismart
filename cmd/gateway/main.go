...
func run() error {
	...
	// чьи правки уехали в отдельные файлы.
	seeded := dir.Seeded()
	if len(seeded.Added)+len(seeded.Updated) > 0 {
		log.Info("профили устройств разложены",
			"добавлено", len(seeded.Added),
			"обновлено", len(seeded.Updated),
			"отложено копией", len(seeded.Forked))
		for _, key := range seeded.Updated {
			log.Info("профиль обновлён из поставки", "профиль", key)
		}
	}

	// Правленый профиль уехал копией, а поставляемый вернулся к эталону.
	// Устройства, настроенные по правке, переводим на копию: связки у них уже
	// заведены и работают, речь только о том, какой профиль откроется в форме
	// устройства и по какому будет пересобрано.
	for _, f := range seeded.Forked {
		moved, err := db.RetargetDevices(ctx, f.Key, f.Copy)
		if err != nil {
			return err
		}
		log.Warn("профиль был правлен вручную: правка отложена в отдельный файл",
			"профиль", f.Key, "копия", f.Copy, "устройств переведено", moved,
			"hint", "поставляемый профиль обновлён и дальше обновляется сам")
	}
